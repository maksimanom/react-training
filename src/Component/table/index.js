...
import usersArrayMain from "../../utils/constants.json";
...
const Table = () => {
  ...
  const [usersArray, setUsersArray] = useState(usersArrayMain);

  const { result, setFilter, setSort, setCurrentPage, currentPage, itemsPerPage, countOfPages, filter } = useTable(usersArray);
  ...
  const [modalSettings, setModalSettings] = useState({ left: 0, top: 0, showed: false, userIndex: -1 });
  ...
  const handleOpenUserWindow = (e, index) => {
    setModalSettings({ left: e.clientX, top: e.clientY, showed: true, userIndex: index });
  }
  ...
  const handleDeleteUser = () => {
    ...
    const secondArrayPart = usersArray.slice(modalSettings.userIndex + 1, usersArray.length);
      ...
      <table className={classes.root}>
        <thead>
          <tr>
            ...
            <td colSpan={headerData.length / 2}>
              <Pagination countOfPages={countOfPages} currentPage={currentPage} itemsPerPage={itemsPerPage} handleChangePage={handleChangePage} />
            </td>
          </tr>
          <tr>
            ...
            <td className={classes.editColumn}><EditIcon /></td>
          </tr>
        </thead>
        <tbody>
          {result.map((item, index) => {
            return (
              <tr key={index}>
                <td><Highlight toFound={filter} prop={item.name} /></td>
                <td><Highlight toFound={filter} prop={item.surname} /></td>
                <td>{item.age}</td>
                <td>{item.working ? "yes" : "no"}</td>
                <td><MoreHorizIcon className={classes.editIcon} onClick={(e) => handleOpenUserWindow(e, itemsPerPage * currentPage + index)} /></td>
              </tr>
            );
          })}
        </tbody>
